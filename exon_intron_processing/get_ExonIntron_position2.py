"""
Given the alignment file it calculates the intron coordinates and saves as csv file
"""
import pandas as pd
import re
import pickle
...
alignment_file = 'ncbiRefSeq.multiz100way.exonNuc.fa' #knownGene.multiz100way.exonNuc.fa
input_file = '/gpfs/commons/home/atalukder/Contrastive_Learning/data/multiz100way/alignment/'+alignment_file  # Replace with the path to your .fa file
...
if alignment_file == 'ncbiRefSeq.multiz100way.exonNuc.fa':
    pattern = re.compile(r"^>(\S+_\S+) (\d+) \d+ \d+ (\S+):(\d+)-(\d+)([+-]?)")
elif alignment_file == 'knownGene.multiz100way.exonNuc.fa':
...
def get_exon_name(identifier):
    ...
    if alignment_file == 'ncbiRefSeq.multiz100way.exonNuc.fa':
        return f"{parts[0]}_{parts[1]}_{parts[3]}_{parts[4]}"
    elif alignment_file == 'knownGene.multiz100way.exonNuc.fa':
...
# df.to_pickle(final_name+'.pkl') # (AT)
