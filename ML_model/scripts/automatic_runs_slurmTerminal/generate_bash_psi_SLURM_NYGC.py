"""
This file submits multiple runs to SLURM (sequentially on one allocation).
"""
import os
import time
import random
...
# --- program bash script (RUN_IDX set by the SLURM loop) ---
...
def create_slurm_file(prg_file_path, job_name, slurm_file_path):
    ...
    header = (
        ...
        f"#SBATCH --time={hour}:45:00\n"
        f"#SBATCH --mem={memory}G\n"
        f"#SBATCH --cpus-per-task={nthred}\n"
        "#SBATCH --mail-type=END,FAIL\n"
        f"#SBATCH --output={output_dir}/out_{job_name}.%j\n"
...
slurm_file_name = 'Psi_mtspliceIntron10augOriginal'
gpu_num = 1
hour = 15
memory = 100 # GB
...
readme_comment = "psi, mtsplice, intron, 10 aug, Original no warm start serial run"
wandb_logger_NOTES="psi mtsplice intron 10 aug Original no warm start serial run"
...
# Experiment layout (shared by all runs of this job)
