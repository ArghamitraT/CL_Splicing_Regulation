...
def plot_all_anchors_from_arrays(
    ...
    def reduce_and_plot(name, coords):
        ...
        df_out = pd.DataFrame({
            ...
            "x": coords[:, 0],
            "y": coords[:, 1],
            "label(1=High,0=Low)": y_bin
        })
        csv_path = os.path.join(out_dir, f"{name}_embeddingWid.csv")
        df_out.to_csv(csv_path, index=False)
        print(f"💾 Saved embedding CSV with exon names → {csv_path}")
...
def main(config: OmegaConf):
    ...
    if SAMPLE_METHOD == "random":
        ...
        # --- Random sampling of up to 500 exons per group ---
        N_SAMPLE = 500
        rng = np.random.default_rng(seed=42)

        def random_sample(embeddings, names, n=N_SAMPLE):
            n = min(n, len(embeddings))
            idx = rng.choice(len(embeddings), size=n, replace=False)
            return np.array(embeddings)[idx], [names[i] for i in idx]

        sampled_high_embeddings, sampled_high = random_sample(all_high_embeddings, high_names)
        sampled_low_embeddings,  sampled_low  = random_sample(all_low_embeddings,  low_names)
