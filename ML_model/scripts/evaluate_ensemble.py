...
# --- Main Ensemble Evaluation Logic (Refactored) ---

def main_evaluate_ensemble(config: DictConfig):
    """Orchestrates the ensemble evaluation using modular functions."""
    
    # Register resolvers if needed (might be redundant if done globally)
    ...
    save_dir = ensemble_base_path / config.ensemble.output_subdir # Define save dir early
    ...
    try:
        ...
        valid_checkpoints = identify_ensemble_checkpoints(
            ensemble_base_path, OmegaConf.to_object(config.ensemble.run_indices) # Convert list from config
        )
        ...
        calculate_and_save_ensemble_metrics(
            averaged_preds_df,
            config.data, # Pass the data config section for GT path
            save_dir,
...
# --- Main execution block (using Hydra Compose API) ---
if __name__ == "__main__":
    # --- Define Ensemble Parameters Here ---
    ensemble_params = OmegaConf.create({
        "ensemble": {
            "base_experiment_path": "exprmnt_2025_10_22__17_47_17/weights/checkpoints/psi_regression/MTsplice/201",
            "run_indices": [1, 2, 3, 4, 5, 6, 7, 8],
    ...
    # --- Load Base Config using Hydra Compose API ---
    with initialize(version_base=None, config_path="../configs"):
        base_cfg = compose(config_name="psi_regression.yaml")
        cfg = OmegaConf.merge(base_cfg, ensemble_params)

        # --- Overrides for Evaluation ---
        cfg.task.gpus = 1
        cfg.data.batch_size = cfg.data.get('val_batch_size', max(cfg.data.batch_size * 2, 128))
        cfg.data.num_workers = get_optimal_num_workers()
        cfg.trainer.max_epochs = 1
        cfg.trainer.logger = None
        cfg.trainer.enable_checkpointing = False
        cfg.aux_models.eval_weights = None
        cfg.aux_models.train_mode = "eval"
        # Ensure correct validation file is used by DataModule
        # Example: cfg.data.val_files.intronexon = "path/to/your/val_file.json"

    # --- Run the main evaluation function ---
    main_evaluate_ensemble(cfg)