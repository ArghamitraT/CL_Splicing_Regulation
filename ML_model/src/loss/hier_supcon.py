...
class weightedSupConLoss(nn.Module):
    ...
    def __init__(self, temperature=0.07, contrast_mode='one',
        ...
        all_exon_names = master_df['exon_id'].tolist()  # ALL HUMAN EXONS
    ...
    def _initialize_dummy_data(self):
        print("Initializing dummy test data...")

        self.name_to_global_idx = {name: i for i, name in enumerate(["exon_0", "exon_1", "exon_2","exon_3","exon_4","exon_5"])}
        num_total_exons = 6
        alt_exon_names = ["exon_0", "exon_1", "exon_2"]

        mad_tensor = torch.tensor([
            [0.0, 0.2, 0.5],
            [0.2, 0.0, 0.3],
            [0.5, 0.3, 0.0]
        ]).float()
        ...
        self.mad_matrix_tensors["train"] = mad_tensor.to(self.device)
        self.global_d_scores["train"] = mad_tensor.to(self.device)
        self.global_to_alt_idx["train"] = global_to_alt.to(self.device)
    ...
    def forward_debug(self, features, exon_name, division,labels=None, mask=None):
        """Compute loss for model. If both `labels` and `mask` are None,
        it degenerates to SimCLR unsupervised loss:
        https://arxiv.org/pdf/2002.05709.pdf
        ...
        print(f"mask_alt_anchor_const_contrast: {mask_alt_anchor_const_contrast.shape}")
        print(f"d_scores_anchors: {d_scores_anchors.shape}")
        weights += mask_alt_anchor_const_contrast * d_scores_anchors.unsqueeze(1)
        weights += mask_const_anchor_alt_contrast * d_scores_contrasts.unsqueeze(0)
